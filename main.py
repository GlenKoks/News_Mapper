...
def build_table(data: pd.DataFrame) -> ft.DataTable:
    ...
    for _, row in data.iterrows():
        dt_value = row.get("dt")
        has_datetime = isinstance(dt_value, pd.Timestamp) and pd.notna(dt_value)
        rows.append(
            ft.DataRow(
                cells=[
                    ft.DataCell(ft.Text(row.get("publication_title_name", ""))),
                    ft.DataCell(ft.Text(dt_value.strftime("%Y-%m-%d") if has_datetime else "—")),
...
def main(page: ft.Page):
    ...
    def update_charts(dataframe: pd.DataFrame):
        topics_series = (
            dataframe.get("topics_verdicts_list", pd.Series(dtype=object))
            .explode()
            .dropna()
            .value_counts()
            .head(10)
        ...
        countries_series = (
            dataframe.get("country", pd.Series(dtype=object)).explode().dropna().value_counts().head(10)
        )
        if countries_series.empty:
            countries_chart_container.content = ft.Text("Страны отсутствуют в выборке")
        else:
            countries_chart_container.content = make_bar_chart(
                countries_series, "Упоминания стран", ft.colors.PURPLE_300
            )

        persons_series = (
            dataframe.get("persons", pd.Series(dtype=object)).explode().dropna().value_counts().head(10)
        )
